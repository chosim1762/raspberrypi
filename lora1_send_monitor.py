...
while True:
    if GPS.in_waiting > 0:
        ...
        if gps_data.startswith('$GPGGA'):
            ...
            monitor["id"] = id
            monitor["id_hex"] = struct.pack('<L', id).hex()
            monitor["now"] = datetime.now().timestamp()
            monitor["now_hex"] = struct.pack('<d', monitor["now"]).hex()
            ...
            counter_hex = struct.pack('<L', counter).hex()
            temp_hex = struct.pack('<f', monitor["TempPR"]).hex()
            pressure_hex = struct.pack('<f', monitor["Pressure"]).hex()
            gx = struct.pack('<f', monitor["Gx"]).hex()
            gy = struct.pack('<f', monitor["Gy"]).hex()
            gz = struct.pack('<f', monitor["Gz"]).hex()

            message = f'{monitor["id_hex"]:<8}{counter_hex:<8}{monitor["now_hex"]:<16}{temp_hex:<8}{pressure_hex:<8}{monitor["latitude_hex"]:<16}{monitor["longitude_hex"]:<16}{gx:<8}{gy:<8}{gz:<8}'
            print(message)
            print(monitor)
